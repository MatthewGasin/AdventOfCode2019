...
public class AdventOfCode {
	...
	public static void main(String[] args) {
		...
		for(String s : inputs) {
			...
			String[] minutes = split[1].replace("]", "").split(":");
			int minute = Integer.parseInt(minutes[0])*60+Integer.parseInt(minutes[1]);
			String log = "";
			for(int i = 2; i < split.length; i++) {
				log += split[i] + " ";
			}
			sortedDates.add(new Date(month, day, minute, log));
		}
		...
		for(Date d : sortedDates) {
			String[] split = d.getLog().split(" ");
			//Each log is either a guard starting their shift, a guard falling asleep, or a guard waking up
			if(split[0].equals("Guard")) {
				//Add the guard!
				int id = Integer.parseInt(split[1].replace("#", ""));
				curGuard = id;
				if(guards.get(id) == null) {
					guards.put(id, new Guard(id));
				}
			//fall asleep
			}else if(split[0].equals("falls")) {
				guards.get(curGuard).fallAsleep(d);
			//Wake up
			}else if(split[0].equals("wakes")) {
				guards.get(curGuard).wakeUp(d);
			}else {
				System.out.println("Messed up log: " + d.getLog());
			}
		}
		
		//Find the id of the guard who slept the most
		int sleptMost = curGuard;
		for(Guard g : guards.values()) {			
			if(g.getTotalSleep() > guards.get(sleptMost).getTotalSleep()) {
				sleptMost = g.getId();
			}
		}
		...
		//PART ONE
		int bestMin = 0;
		int[] napTime = new int[60];
		for(int i = 0; i < napTime.length; i++) {
			napTime[i] = 0;
		}
		for(Date[] naps : guards.get(sleptMost).getSleepTimes()) {
			for(int i = naps[0].minute; i < naps[1].minute; i++) {
				napTime[i]++;
			}
		}
		for(int i = 0; i < napTime.length; i++) {
			if(napTime[i] >= napTime[bestMin]) {
				bestMin = i;
			}
		}
		
		result = sleptMost * bestMin;
		System.out.println(result);
		
		
		//PART TWO
		//Find the most frequent nap time
		int bestGuard = 0, mostFreq = 0, mostFreqMinute = 0;
		for(Guard g : guards.values()) {
			g.correctNapTime();
			for(int i = 0; i < g.napTime.length; i++) {
				if(g.napTime[i] > mostFreq) {
					bestGuard = g.getId();
					mostFreq = g.napTime[i];
					mostFreqMinute = i;
				}
			}
		}
		
		result = bestGuard * mostFreqMinute;
		System.out.println(result);
	}


	private static String getInput() {
		return "[1518-09-14 00:54] wakes up\r\n" + 
